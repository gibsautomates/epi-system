-- ============================================
-- ADD LOWEST VENDOR TO ENHANCED VIEW
-- Extracts the vendor with the lowest price
-- ============================================
...
lowest_vendor_calc AS (
    -- Calculate lowest vendor from vendor offers
    SELECT DISTINCT ON (vo.upc)
        vo.upc,
        vo.vendor_name as lowest_vendor,
        vo.qty
    FROM vendor_offers vo
    INNER JOIN vendor_files vf ON vo.file_id = vf.id
    INNER JOIN (
        -- Get minimum price for each UPC
        SELECT vo2.upc, MIN(vo2.price) as min_price
        FROM vendor_offers vo2
        INNER JOIN vendor_files vf2 ON vo2.file_id = vf2.id
        WHERE vf2.file_date >= CURRENT_DATE - INTERVAL '21 days'
            AND vo2.vendor_type = 'DOMESTIC'
        GROUP BY vo2.upc
    ) min_prices ON vo.upc = min_prices.upc AND vo.price = min_prices.min_price
    WHERE vf.file_date >= CURRENT_DATE - INTERVAL '21 days'
        AND vo.vendor_type = 'DOMESTIC'
    ORDER BY 
        vo.upc,
        CASE 
            WHEN vo.qty IS NULL THEN 0  -- NULL qty means plenty, so prioritize it
            ELSE 1 
        END,
        vo.qty DESC NULLS FIRST,  -- Then by highest qty (NULL first as it means unlimited)
        vo.vendor_name  -- Finally alphabetically as tiebreaker
)
SELECT 
    nd.*,
