-- ============================================
-- WATCHLIST TRACKING TABLE (FIXED VERSION)
...
CREATE TABLE IF NOT EXISTS watchlist_tracking (
    ...
    upc TEXT NOT NULL,  -- Changed from BIGINT to TEXT to handle all UPC formats
...
BEGIN
    ...
    SELECT 
        CAST(nd.upc AS TEXT),  -- Cast UPC to TEXT
        CASE 
    ...
    WHERE nd.upc IS NOT NULL  -- Ensure UPC is not null
...
WITH item_details AS (
    ...
    WHERE vol.upc IS NOT NULL AND vol.price > 0  -- Filter out invalid data
),
item_aggregates AS (
    SELECT 
        upc,
        ...
        MAX(brand) as brand,
        MAX(sku) as sku,
        MIN(price) as lowest_price,  -- Renamed from min_price
        ...
        (SELECT vendor FROM item_details id2 
         WHERE id2.upc = item_details.upc AND id2.price_rank = 1 
         LIMIT 1) as lowest_vendor
    FROM item_details
    GROUP BY upc
),
latest_tracking AS (
...
LEFT JOIN latest_tracking lt ON CAST(ia.upc AS TEXT) = lt.upc
LEFT JOIN previous_tracking pt ON CAST(ia.upc AS TEXT) = pt.upc
LEFT JOIN price_trends pt2 ON ia.upc = pt2.upc
WHERE ia.upc IS NOT NULL  -- Ensure we only include valid UPCs
ORDER BY ia.domestic_vendor_count DESC, ia.lowest_price ASC;

-- Step 4: Initialize tracking with current data (with error handling)
...
WHERE upc IS NOT NULL AND min_price IS NOT NULL  -- Only valid records
ON CONFLICT (upc, snapshot_date) DO NOTHING;
...
-- Step 6: Test the view with limited results
SELECT 
    upc,
    item_name,
    status,
